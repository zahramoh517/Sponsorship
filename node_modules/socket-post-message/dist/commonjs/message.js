...
exports.Reader = exports.message = void 0;
...
const message = (value) => {
    ...
    const msg = (0, node_v8_1.serialize)(value);
...
class Reader {
    ...
    write(chunk) {
        ...
        let chunkOffset = 0;
        while (true) {
            const messageLength = getMessageLength(this.#buffer, this.#offset, chunk, chunkOffset);
            if (messageLength === -1) {
            ...
            const haveAll = chunk.length - chunkOffset + rem >= messageLength + 4;
            if (!haveAll) {
                this.#buffer = !this.#buffer
                    ? chunk
                    : Buffer.concat([
            ...
            let msg;
            if (rem <= 0) {
                msg = chunk.subarray(chunkOffset + 4, chunkOffset + 4 + messageLength);
            }
            else if (rem <= 4) {
                ...
                const m2 = chunk.subarray(chunkOffset, messageLength - rem + 4);
                msg = Buffer.concat([m1, m2]);
            }
            chunkOffset += 4 - rem + messageLength;
            this.#buffer = undefined;
            messages.push((0, node_v8_1.deserialize)(msg));
        }
    }
}
exports.Reader = Reader;
const getMessageLength = (b1, offset1, b2, offset2) => {
    ...
    const a = (!b1 || inB1 <= 0 ? b2[offset2 + 0 - inB1] : b1[offset1 + 0]);
    const b = (!b1 || inB1 <= 1 ? b2[offset2 + 1 - inB1] : b1[offset1 + 1]);
    const c = (!b1 || inB1 <= 2 ? b2[offset2 + 2 - inB1] : b1[offset1 + 2]);
    const d = (!b1 || inB1 <= 3 ? b2[offset2 + 3 - inB1] : b1[offset1 + 3]);
