 ...
 */
export declare function catcher<F extends (...a: any[]) => any>(fn: F): FindReturnType<OverloadMap<F>, []> | undefined;
export declare function catcher<F extends (...a: any[]) => any, E>(fn: F, caughtValue: E): FindReturnType<OverloadMap<F>, []> | E;
/**
 ...
 */
export declare function catchWrap<F extends (...a: any[]) => any>(fn: F): AddReturnType<F, undefined>;
export declare function catchWrap<F extends (...a: any[]) => any, E>(fn: F, caughtValue: E): AddReturnType<F, E>;
/**
...
export type OverloadMapRaw<F> = F extends {
    ...
    (...a: infer A0): infer R0;
    (...a: infer A1): infer R1;
} ? [[NeverUnknown<A0>, R0], [NeverUnknown<A1>, R1]] : F extends (...a: infer A0) => infer R0 ? [[NeverUnknown<A0>, R0]] : never;
/**
 * The same as {@link OverloadMapRaw}, but with the `[never, unknown]`
 * entries filtered out.
 */
export type OverloadMap<F extends (...a: any[]) => any> = FilterNeverMap<OverloadMapRaw<F>>;
/**
 * Filter [never,unknown] out of a list
 */
export type FilterNeverMap<L> = L extends [h: infer H, ...t: infer T] ? [never, unknown] extends H ? FilterNeverMap<T> : [H, ...FilterNeverMap<T>] : [];
/**
...
export type FindReturnType<M, P extends unknown[]> = M extends [
    h: infer H,
    ...t: infer T
] ? H extends [P, infer R] ? R : FindReturnType<T, P> : never;
/**
...
export type AddReturnTypeToOverloadMap<L, A = undefined> = L extends [
    infer H,
    ...infer T
] ? H extends [unknown[], unknown] ? [[H[0], H[1] | A], ...AddReturnTypeToOverloadMap<T, A>] : [] : [];
/**
 * Add a given return type to all function signatures of a function type
 */
type AddReturnType<F extends (...a: any) => any, A = undefined> = MakeOverloadFunction<AddReturnTypeToOverloadMap<OverloadMap<F>, A>>;
/**
 * Convert a filtered overload map back into a function type
 */
type MakeOverloadFunction<L extends [unknown[], unknown][]> = L extends [infer H, ...infer T] ? H extends [unknown[], unknown] ? {
    (...p: H[0]): H[1];
